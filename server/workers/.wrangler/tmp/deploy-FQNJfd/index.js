  ...
  "node_modules/@supabase/realtime-js/dist/module/lib/push.js"() {
    ...
    Push = class {
      ...
      startTimeout() {
        ...
        const callback = /* @__PURE__ */ __name((payload2) => {
          this._cancelRefEvent();
          this._cancelTimeout();
          this.receivedResp = payload2;
          this._matchReceive(payload2);
        }, "callback");
    ...
    RealtimeChannel = class _RealtimeChannel {
          ...
          (_b = this.bindings[typeLower]) === null || _b === void 0 ? void 0 : _b.filter((bind) => {
            ...
            if (["broadcast", "presence", "postgres_changes"].includes(typeLower)) {
              if ("id" in bind) {
                ...
                return bindId && ((_b2 = payload2.ids) === null || _b2 === void 0 ? void 0 : _b2.includes(bindId)) && (bindEvent === "*" || (bindEvent === null || bindEvent === void 0 ? void 0 : bindEvent.toLocaleLowerCase()) === ((_c = payload2.data) === null || _c === void 0 ? void 0 : _c.type.toLocaleLowerCase()));
              } else {
                const bindEvent = (_e = (_d = bind === null || bind === void 0 ? void 0 : bind.filter) === null || _d === void 0 ? void 0 : _d.event) === null || _e === void 0 ? void 0 : _e.toLocaleLowerCase();
                return bindEvent === "*" || bindEvent === ((_f = payload2 === null || payload2 === void 0 ? void 0 : payload2.event) === null || _f === void 0 ? void 0 : _f.toLocaleLowerCase());
              }
            } else {
      ...
      _getPayloadRecords(payload2) {
        ...
        if (payload2.type === "INSERT" || payload2.type === "UPDATE") {
          records.new = convertChangeData(payload2.columns, payload2.record);
        }
        if (payload2.type === "UPDATE" || payload2.type === "DELETE") {
          records.old = convertChangeData(payload2.columns, payload2.old_record);
        }
        return records;
      }
    };
...
Option 2: Install and provide the "ws" package:
  ...
  const client = new RealtimeClient(url, {
    ...
    transport: ws
       ...
       */
      push(data) {
        const { topic, event, payload: payload2, ref } = data;
        ...
        this.log("push", `${topic} ${event} (${ref})`, payload2);
        if (this.isConnected()) {
          callback();
        } else {
      ...
      _onConnMessage(rawMessage) {
        this.decode(rawMessage.data, (msg) => {
          ...
          const { topic, event, payload: payload2, ref } = msg;
          const refString = ref ? `(${ref})` : "";
          const status = payload2.status || "";
          this.log("receive", `${status} ${topic} ${event} ${refString}`.trim(), payload2);
          this.channels.filter((channel) => channel._isMember(topic)).forEach((channel) => channel._trigger(event, payload2, ref));
      ...
      async _performAuth(token = null) {
        ...
        if (this.accessTokenValue != tokenToSend) {
          ...
          this.channels.forEach((channel) => {
            ...
            tokenToSend && channel.updateJoinPayload(payload2);
      ...
      _initializeOptions(options) {
        ...
        switch (this.vsn) {
          case VSN_1_0_0:
            this.encode = (_j = options === null || options === void 0 ? void 0 : options.encode) !== null && _j !== void 0 ? _j : ((payload2, callback) => {
              return callback(JSON.stringify(payload2));
            });
            this.decode = (_k = options === null || options === void 0 ? void 0 : options.decode) !== null && _k !== void 0 ? _k : ((payload2, callback) => {
...
var StorageError, StorageApiError, StorageUnknownError, resolveFetch$1, resolveResponse$1, recursiveToCamel, isPlainObject$1, isValidBucketName, _getErrorMessage$1, handleError$1, _getRequestParams$1, StreamDownloadBuilder, _Symbol$toStringTag, BlobDownloadBuilder, DEFAULT_SEARCH_OPTIONS, DEFAULT_FILE_OPTIONS, StorageFileApi, version2, DEFAULT_HEADERS$1, StorageBucketApi, StorageAnalyticsClient, DEFAULT_HEADERS, StorageVectorsError, StorageVectorsApiError, StorageVectorsUnknownError, resolveFetch2, isPlainObject, _getErrorMessage, handleError, _getRequestParams, VectorIndexApi, VectorDataApi, VectorBucketApi, StorageVectorsClient, VectorBucketScope, VectorIndexScope, StorageClient;
var init_dist3 = __esm({
  "node_modules/@supabase/storage-js/dist/index.mjs"() {
    ...
    resolveResponse$1 = /* @__PURE__ */ __name(() => {
      return Response;
    }, "resolveResponse$1");
    ...
    isPlainObject$1 = /* @__PURE__ */ __name((value) => {
    ...
    handleError$1 = /* @__PURE__ */ __name(async (error, reject, options) => {
    ...
    version2 = "2.89.0";
    DEFAULT_HEADERS$1 = { "X-Client-Info": `storage-js/${version2}` };
    ...
    handleError = /* @__PURE__ */ __name(async (error, reject, options) => {
    ...
    GoTrueClient = class _GoTrueClient {
      ...
      async _setSession(currentSession) {
        try {
          ...
          let hasExpired = true;
          let session = null;
          const { payload: payload2 } = decodeJWT(currentSession.access_token);
          if (payload2.exp) {
            expiresAt2 = payload2.exp;
      ...
      async _getAuthenticatorAssuranceLevel() {
        ...
        const { payload: payload2 } = decodeJWT(session.access_token);
      ...
      async getClaims(jwt, options = {}) {
        try {
          ...
          const { header, payload: payload2, signature, raw: { header: rawHeader, payload: rawPayload } } = decodeJWT(token);
          if (!(options === null || options === void 0 ? void 0 : options.allowExpired)) {
            validateExp(payload2.exp);
          }
...
var init_auth = __esm({
  "src/services/auth.ts"() {
    ...
    AuthService = class _AuthService {
      ...
      static SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZ6Y3B5eWJmbmxxbGlzZHFlcmNnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjQ5MTc1NjQsImV4cCI6MjA4MDQ5MzU2NH0.sE1-WHRNtxdJiDITqVFl8e_jSINtcUn_WVqehOmK-HY";
      constructor(env) {
        const supabaseUrl = env.SUPABASE_URL || "https://fzcpyybfnlqlisdqercg.supabase.co";
        const supabaseKey = env.SUPABASE_SERVICE_KEY || _AuthService.SUPABASE_ANON_KEY;
      ...
      async verifyToken(token) {
        ...
        try {
          ...
          return {
            ...
            exp: 0
            // Supabase handles expiration, we just assume valid if getUser succeeds
          };
        } catch (err) {
      ...
      static async getAuthUser(c) {
        ...
        if (!token) return null;
        const authService = new _AuthService(c.env);
...
var DatabaseService = class {
   ...
   */
  async ensureUserExists(userId, email, name, role) {
    const existing = await this.db.prepare("SELECT id FROM users WHERE id = ?").bind(userId).first();
    if (existing) return;
    ...
    await this.db.prepare(`
      ...
      email,
      "supabase-managed",
      // Placeholder - password is managed by Supabase
      name || email.split("@")[0],
      userRole,
      tamuuId,
      now,
   ...
   * Get user's Tamuu ID
   */
  async getUserTamuuId(userId) {
    const result = await this.db.prepare("SELECT tamuu_id FROM users WHERE id = ?").bind(userId).first();
   ...
   */
  async generateTamuuIdsForExistingUsers() {
    const errors = [];
    let updated = 0;
    const result = await this.db.prepare("SELECT id, role FROM users WHERE tamuu_id IS NULL").all();
    const users = result.results || [];
    console.log(`[DB] Found ${users.length} users without Tamuu ID`);
    for (const user of users) {
      try {
        const tamuuId = this.generateTamuuId(user.role);
        await this.db.prepare("UPDATE users SET tamuu_id = ? WHERE id = ?").bind(tamuuId, user.id).run();
        updated++;
   ...
   */
  async canUserAccessTemplate(templateId, userId, isAdmin) {
    if (isAdmin) return true;
    const template = await this.db.prepare("SELECT user_id FROM templates WHERE id = ?").bind(templateId).first();
  ...
  async getTemplate(id) {
    ...
    return {
      ...
      invitationMessage: template.invitation_message || void 0,
  ...
  async upsertSection(templateId, sectionType, updates) {
    ...
    if (existing) {
      ...
      if (updates.elements) {
        await this.db.prepare("DELETE FROM template_elements WHERE section_id = ?").bind(existing.id).run();
  ...
  async checkSlugAvailability(slug) {
    const existing = await this.db.prepare("SELECT id FROM templates WHERE slug = ?").bind(slug.toLowerCase()).first();
    return !existing;
  }
  /**
  ...
  async getTemplateBySlug(slug) {
    const template = await this.db.prepare("SELECT id FROM templates WHERE slug = ?").bind(slug.toLowerCase()).first();
    if (!template) return null;
   ...
   */
  async getMasterTemplates(category) {
    let query = `
            SELECT id, user_id, name, slug, category, source_template_id, thumbnail, status, updated_at, created_at
            FROM templates
   ...
   */
  async cloneTemplateForUser(sourceTemplateId, userId, slug, name, category) {
    ...
    Object.values(cleanedSections).forEach((section) => {
      delete section.id;
    ...
    const newTemplate = await this.createTemplate({
      ...
      status: "published",
      // Default to published for immediate visibility
      sectionOrder: source.sectionOrder,
      customSections: source.customSections,
      globalTheme: source.globalTheme,
      eventDate: source.eventDate,
      sections: cleanedSections
      // This will create all sections + elements with NEW IDs
    });
    return newTemplate;
  }
  // ============================================
  ...
  async getGuests(invitationId) {
    const results = await this.db.prepare("SELECT * FROM guests WHERE invitation_id = ? ORDER BY created_at DESC").bind(invitationId).all();
    return (results.results || []).map((g) => this.mapGuestToResponse(g));
  }
  /**
   * Add a single guest
   */
  async addGuest(invitationId, data) {
    const id = generateUUID();
    ...
    const created = await this.db.prepare("SELECT * FROM guests WHERE id = ?").bind(id).first();
  ...
  async updateGuest(guestId, updates) {
    ...
    if (updates.guestCount !== void 0) {
      sets.push("guest_count = ?");
      values.push(updates.guestCount);
    }
    if (updates.checkedInAt !== void 0) {
      sets.push("checked_in_at = ?");
      values.push(updates.checkedInAt);
    }
    values.push(guestId);
    await this.db.prepare(`UPDATE guests SET ${sets.join(", ")} WHERE id = ?`).bind(...values).run();
  }
  /**
   * Delete guest
   */
  async deleteGuest(guestId) {
    await this.db.prepare("DELETE FROM guests WHERE id = ?").bind(guestId).run();
  }
  /**
   * Bulk add guests
   * Enterprise implementation using D1 batch for high performance
   */
  async bulkAddGuests(invitationId, guests2) {
    if (guests2.length === 0) return;
    const now = (/* @__PURE__ */ new Date()).toISOString();
    const statements = guests2.map((g) => {
      const id = generateUUID();
      const checkInCode = Math.random().toString(36).substring(2, 8).toUpperCase();
...
authRouter.get("/me", async (c) => {
  ...
  const authUser = await AuthService2.getAuthUser(c);
  if (!authUser) {
    return c.json({ error: "Not authenticated" }, 401);
  }
  const user = await c.env.DB.prepare("SELECT * FROM users WHERE id = ?").bind(authUser.userId).first();
  if (!user) {
...
authRouter.put("/profile", async (c) => {
  ...
  await c.env.DB.prepare(`
            ...
            SET name = COALESCE(?, name), 
                ...
                updated_at = ?
            WHERE id = ?
        `).bind(name, phone, avatarUrl, gender, birthDate, now, authUser.userId).run();
...
init_auth();
var invitationsRouter = new Hono2();
var SLUG_REGEX = /^[a-z0-9][a-z0-9-]{4,48}[a-z0-9]$/;
function isValidSlugFormat(slug) {
  return SLUG_REGEX.test(slug);
}
__name(isValidSlugFormat, "isValidSlugFormat");
...
app.route("/api/guests", guests_default);
